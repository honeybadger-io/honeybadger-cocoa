...
- The thread-introspection hook is inert in forked children: libsystem re-fires it during atfork-child re-init, between `fork()` and `exec`, where its `malloc` intermittently aborted the child (`os_unfair_lock is corrupt`) in any host app that forks — e.g. spawning a terminal via `forkpty`. The hook now compares `getpid()` against the installing process and only chains to a predecessor when they differ.
- Per-thread alternate signal stacks are actually freed at thread exit (via a TSD destructor). The previous cleanup ran in the `THREAD_TERMINATE` introspection event — after libpthread has already cleared thread-specific data — so it never fired and every short-lived thread leaked its 64KB stack; its `sigaltstack(SS_DISABLE)` call was also rejected by the kernel (zero `ss_size`).
